...
class LM_LSTM_CRF(nn.Module):
    ...
    def __init__(self, w_weight,tagset_size, char_size, char_dim, char_hidden_dim, char_rnn_layers, embedding_dim, word_hidden_dim, word_rnn_layers, vocab_size, dropout_ratio, large_CRF=True, if_highway = False, in_doc_words = 2, highway_layers = 1):
        ...
        self.word_lstm = nn.LSTM(char_hidden_dim * 2+embedding_dim, word_hidden_dim // 2, num_layers=word_rnn_layers, bidirectional=True, dropout=dropout_ratio)
    ...
    def forward(self, forw_sentence, forw_position, back_sentence, back_position, word_seq,word_to_next, prior_prob, hidden=None):
        '''
        args:
        ...
        #word
        word_emb = self.word_embeds(word_to_next)*self.word_weight
        d_word_emb = self.dropout(word_emb)
        #d_char_out=d_char_out*(1-self.word_weight)

        #combine
        word_input = torch.cat((d_word_emb, d_char_out), dim = 2)
        #word_input=d_char_out
        #word level lstm
        lstm_out, _ = self.word_lstm(word_input)
        d_lstm_out = self.dropout(lstm_out)

        #convert to crf
        crf_out = self.crf(d_lstm_out, prior_prob)
