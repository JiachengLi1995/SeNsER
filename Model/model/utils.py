...
def covert_prob2feature(l_map, prob):
    target_size = len(l_map)
    prob_feature = np.zeros([len(prob), target_size]) ## seq_len, target_size
    for i in range(len(prob)):
        one_prob = prob[i]
        for sub_one_prob in one_prob:
            full_text, p, is_start = sub_one_prob
            label = '_'.join(full_text.split())
            if is_start==1:
                label = 'B-'+label
            else:
                label = 'I-'+label
            label_idx = l_map[label]
            prob_feature[i][label_idx] = p
    return prob_feature


def clean_prob(s, prob):
    removed_s = ['ap&m', 'ebu3b']
    for r_s in removed_s:
        index = s.find(r_s)
        if index!=-1:
            for i in range(index, index+len(r_s)):
                prob[i] = []
    return prob

def generate_prior_prob(r_c_map, l_map, feature, knowledge_dict):
    sentences = convert_feature2char(r_c_map, feature)
    seq_len = feature.size()[0]
    batch_size = feature.size()[1]
    target_size = len(l_map)
    prior_prob = np.zeros([batch_size, seq_len, target_size])
    #r_l_map = revlut(l_map)
    for i, s in enumerate(sentences):
        ...
        while (s.count('\n')): 
            s.remove('\n')
        s = ''.join(s)
        if s in knowledge_dict:
            #print(len(s))
            prob = clean_prob(s, knowledge_dict[s])
            prob_feature = covert_prob2feature(l_map, prob)
            prior_prob[i, :prob_feature.shape[0], :] = prob_feature
            # for j in range(prob_feature.shape[0]):
            #     if (prob_feature[j]>0).sum() >0:
            #         _, idx = torch.max(torch.tensor(prob_feature[j]), dim=0)
                    #print(r_l_map[int(idx.data)], end=' ')
            #print()
    prior_prob = torch.tensor(prior_prob).transpose(0, 1).contiguous()
    if torch.cuda.is_available():
        prior_prob = prior_prob.cuda()
    #exit(0)
    return prior_prob  #seq_len, batch_size, target_size

def convert_feature2char(r_c_map, feature):
    feature = feature.transpose(0, 1).cpu().data.numpy() #(batch, seq_len)
    sentences = []

    for i in range(feature.shape[0]):
        sentence = []
        for j in range(feature.shape[1]):
            sentence.append(r_c_map[feature[i][j]])
        sentences.append(sentence)
        sentence = []
    
    return sentences
    
def string_match_embedding(path,emb_size):
    emb=PyTrie()
    emb.insert_from_file(path.encode('utf-8'),ord(' '),False)
    word_to_id=dict()
    indoc_embedding_array = list()
    for i in range(len(open(path,'r').readlines())+1):
         
         word_to_id[emb.get_word(i).decode('utf-8')]=i
    for line in open(path, 'r'):
          
        line = line.split(' ')
        vector = list(map(lambda t: float(t), filter(lambda n: n and not n.isspace(), line[1:])))
        indoc_embedding_array.append(vector)           
    rand_embedding_tensor_1 = torch.FloatTensor(1, emb_size)
    rand_embedding_tensor_2=torch.FloatTensor(1,emb_size)
    embedding_tensor_0 = torch.FloatTensor(np.asarray(indoc_embedding_array))
    embedding_tensor = torch.cat([rand_embedding_tensor_1, embedding_tensor_0,rand_embedding_tensor_2], 0)
    word_to_id['<eof>']=len(word_to_id.keys())
    return word_to_id,embedding_tensor,emb


def reconstruct_word_input(orig_w,orig_dict,emb,w_to_id,gpu):
    orig_id_to_word={i[1]:i[0] for i in orig_dict.items()}
    w_list=[[i[j] for i in orig_w.tolist()] for j in range(len(orig_w.tolist()[0]))]
    new_word=[]
    sen_len=len(w_list[0])
    for onesentence in w_list:
        newstr=""
        for i in onesentence:
            if orig_id_to_word[i]!='<eof>':
                if len(orig_id_to_word[i])>1:newstr+='@'
                else: newstr+=orig_id_to_word[i]
        newstr=re.sub("[0-9]","#",newstr)
        
        matches=emb.match_whole_string(newstr.encode('utf-8'))
        pad=[w_to_id['<eof>']]*(sen_len-len(matches))
        new_word.append(matches+pad)
    
    new_word=[[i[j] for i in new_word] for j in range(len(new_word[0]))]  
    

    
    new_word=torch.from_numpy(np.array(new_word))
    if gpu>=0:
        new_word=new_word.cuda()
    return new_word
